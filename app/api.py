...
def create_payment_method(request: create_payment_method_req, db: Session = Depends(get_db)):
    ...
    if is_tarjeta:
        ...
        if not request.marca_pago_id: missing.append("marca_pago_id")
        if not request.nro_tarjeta: missing.append("nro_tarjeta")
        if not request.nombre_titular: missing.append("nombre_titular")
        if not request.fecha_vencimiento: missing.append("fecha_vencimiento")
        if not request.banco_id: missing.append("banco_id")
    ...
    if is_billetera:
        ...
        missing = []
        if not request.gateway_pago_id: missing.append("gateway_pago_id")
        if not request.identificador_wallet: missing.append("identificador_wallet")
        if not request.moneda_id: missing.append("moneda_id")
        if missing:
...
def update_payment_method(pm_id: int, request: update_payment_method_req, db: Session = Depends(get_db)):
    ...
    if is_tarjeta:
        ...
        missing = []
        marca_pago_id = request.marca_pago_id or (detalle_tarjeta_actual.marca_pago_id if detalle_tarjeta_actual else None)
        if not marca_pago_id:
            missing.append("marca_pago_id")

        nro_tarjeta_val = request.nro_tarjeta or (detalle_tarjeta_actual.numero_tarjeta if detalle_tarjeta_actual else None)
        if not nro_tarjeta_val:
            missing.append("nro_tarjeta")

        nombre_titular = request.nombre_titular or (detalle_tarjeta_actual.nombre_titular if detalle_tarjeta_actual else None)
        if not nombre_titular:
            missing.append("nombre_titular")

        fecha_vencimiento = request.fecha_vencimiento or (detalle_tarjeta_actual.fecha_vencimiento if detalle_tarjeta_actual else None)
        if not fecha_vencimiento:
            missing.append("fecha_vencimiento")

        banco_id = request.banco_id or (detalle_tarjeta_actual.banco_id if detalle_tarjeta_actual else None)
        if not banco_id:
        ...
        missing = []
        gateway_id = request.gateway_pago_id or (detalle_billetera_actual.proveedor_id if detalle_billetera_actual else None)
        if not gateway_id:
            missing.append("gateway_pago_id")

        wallet_id = request.identificador_wallet or (detalle_billetera_actual.wallet_id if detalle_billetera_actual else None)
        if not wallet_id:
            missing.append("identificador_wallet")

        moneda_id = request.moneda_id or (detalle_billetera_actual.moneda_id if detalle_billetera_actual else metodo_pago.moneda_id)
        if not moneda_id:
    ...
    if detalle_tarjeta:
        marca_metodo: MarcaMetodoPago | None = db.query(MarcaMetodoPago).filter(MarcaMetodoPago.id == detalle_tarjeta.marca_pago_id).first()
    ...
    if detalle_billetera:
        gateway_metodo: ProveedorBilletera | None = db.query(ProveedorBilletera).filter(ProveedorBilletera.id == detalle_billetera.proveedor_id).first()
...
def delete_payment_method(pm_id: int, db: Session = Depends(get_db)):
    ...
    mp_detalle = metodo_pago.metodo_pago_detalle_id 
    mp_tipo = metodo_pago.tipo_pago_id

    tipo: TipoMetodoPago = db.query(TipoMetodoPago).filter(TipoMetodoPago.id == mp_tipo).first()

    if tipo.nombre_metodo == "billetera":

        mp_detalle: MetodoPagoBilletera = db.query(MetodoPagoBilletera).filter(MetodoPagoBilletera.id == mp_detalle)
        mp_detalle.fecha_hora_baja = datetime.utcnow()

    if tipo.nombre_metodo == "tarjeta":

        mp_detalle: MetodoPagoTarjeta = db.query(MetodoPagoTarjeta).filter(MetodoPagoTarjeta)
        mp_detalle.fecha_hora_baja = datetime.utcnow()
